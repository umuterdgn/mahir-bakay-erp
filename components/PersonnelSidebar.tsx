    ...
    <>
      ...
      <div className="lg:hidden fixed top-0 left-0 right-0 bg-white dark:bg-slate-900 border-b border-slate-200 dark:border-slate-800 z-40 p-4">
        <div className="flex items-center justify-between">
          ...
          <h1 className="text-slate-900 dark:text-white font-semibold">Şantiye Asistanı</h1>
      ...
      >
        <div className={`flex flex-col h-full ${isCollapsed ? "p-3" : ""}`}>
          {/* Header */}
          <div className={`p-6 border-b border-slate-200 dark:border-slate-800 ${isCollapsed ? "hidden" : ""}`}>
            <div className="flex items-center gap-3">
              ...
              {!isCollapsed && (
                <div className="flex-1">
                  ...
                  <p className="text-slate-600 dark:text-slate-400 text-xs">Mahir Bakay Mühendislik</p>
                </div>
              )}
              <button
                onClick={() => setIsCollapsed(!isCollapsed)}
                className="hidden lg:flex text-slate-600 dark:text-slate-400 hover:text-slate-900 dark:hover:text-white p-1"
              >
            ...
            <div className="p-4 border-b border-slate-200 dark:border-slate-800">
              <div className="flex items-center gap-3">
                ...
                <div className="flex-1 min-w-0">
                  <p className="text-slate-900 dark:text-white font-medium truncate">{session?.user?.name || "Personel"}</p>
                  <p className="text-slate-600 dark:text-slate-400 text-xs truncate">Saha Mühendisi</p>
                </div>
              </div>
            </div>
          )}
          ...
          {/* Navigation */}
          <nav className={`flex-1 ${isCollapsed ? "p-3" : "p-4"} space-y-1 overflow-y-auto`}>
            {navItems.map((item) => {
              ...
              return (
                <Link
                  ...
                  href={item.href}
                  onClick={() => setIsOpen(false)}
                  className={`flex items-center w-full py-3 rounded-lg transition-all ${
                    isActive
                      ? "bg-blue-50 dark:bg-slate-800 text-blue-600 dark:text-white shadow-lg shadow-blue-600/20"
          ...
          {/* Footer */}
          <div className={`p-4 border-t border-slate-200 dark:border-slate-800 ${isCollapsed ? "p-3" : ""}`}>
            <button
              onClick={() => signOut({ callbackUrl: '/login' })}
              className={`w-full px-4 py-3 rounded-lg bg-red-50 dark:bg-red-600/10 hover:bg-red-600 transition-all text-red-600 dark:text-red-400 hover:text-white flex items-center gap-3 ${
